...
//1��16λ�����Ϊ2��8λ��
//DataLΪ��λ��DataHΪ��λ
void SCServo::Host2STS(u8 *DataL, u8* DataH, int Data)
{
	if(End){
        	*DataH = (Data>>8);
		*DataL = (Data&0xff);
...
//2��8λ�����Ϊ1��16λ��
//DataLΪ��λ��DataHΪ��λ
int SCServo::STS2Host(u8 DataL, u8 DataH)
{
	int Data;
	if(End){
...
int SCServo::sc_writeWord(u8 ID, u8 MemAddr, u16 wDat)
{
	flushSerial();
	u8 buf[2];
...
int SCServo::st_writeWord(u8 ID, u8 MemAddr, u16 wDat)
{
	flushSerial();
	u8 buf[2];
	Host2STS(buf+0, buf+1, wDat);
	writeBuf(ID, MemAddr, buf, 2, INST_WRITE);
	return Ack(ID);
}
...
int SCServo::sc_writePos(u8 ID, u16 Position, u16 Time, u16 Speed, u8 Fun)
{
	flushSerial();
	u8 buf[6];
...
int SCServo::st_writePos(u8 ID, u16 Position, u16 Time, u16 Speed, u8 Fun)
{
	flushSerial();
	u8 buf[6];
	Host2STS(buf+0, buf+1, Position);
	Host2STS(buf+2, buf+3, Time);
	Host2STS(buf+4, buf+5, Speed);
	writeBuf(ID, P_GOAL_POSITION_L, buf, 6, Fun);
	return Ack(ID);
}
...
int SCServo::sc_WritePos(u8 ID, u16 Position, u16 Time, u16 Speed)
{
	return sc_writePos(ID, Position, Time, Speed, INST_WRITE);
}
int SCServo::st_WritePos(u8 ID, u16 Position, u16 Time, u16 Speed)
{
	return st_writePos(ID, Position, Time, Speed, INST_WRITE);
}

//�첽дλ��ָ��
//���ID��Positionλ�ã�ִ��ʱ��Time��ִ���ٶ�Speed
int SCServo::sc_RegWritePos(u8 ID, u16 Position, u16 Time, u16 Speed)
{
	return sc_writePos(ID, Position, Time, Speed, INST_REG_WRITE);
}

int SCServo::st_RegWritePos(u8 ID, u16 Position, u16 Time, u16 Speed)
{
	return st_writePos(ID, Position, Time, Speed, INST_REG_WRITE);
}
...
void SCServo::sc_SyncWritePos(u8 ID[], u8 IDN, u16 Position, u16 Time, u16 Speed)
{
	u8 buf[6];
	Host2SCS(buf+0, buf+1, Position);
	Host2SCS(buf+2, buf+3, Time);
...
void SCServo::st_SyncWritePos(u8 ID[], u8 IDN, u16 Position, u16 Time, u16 Speed)
{
	u8 buf[6];
	Host2STS(buf+0, buf+1, Position);
	Host2STS(buf+2, buf+3, Time);
...
//��2�ֽڣ���ʱ����-1
int SCServo::sc_readWord(u8 ID, u8 MemAddr)
{	
	u8 nDat[2];
	int Size;
	u16 wDat;
...
int SCServo::st_readWord(u8 ID, u8 MemAddr)
{	
	u8 nDat[2];
	int Size;
	u16 wDat;
	...
	if(Size!=2)
		return -1;
	wDat = STS2Host(nDat[0], nDat[1]);
	return wDat;
}

//��λ�ã���ʱ����-1
int SCServo::sc_ReadPos(u8 ID)
{
	return sc_readWord(ID, P_PRESENT_POSITION_L);
}
int SCServo::st_ReadPos(u8 ID)
{
	return st_readWord(ID, P_PRESENT_POSITION_L);
}

//��Ȧ����ָ��
int SCServo::sc_WriteSpe(u8 ID, s16 Speed)
{
	if(Speed<0){
		Speed = -Speed;
		Speed |= (1<<10);
	}
	return sc_writeWord(ID, P_GOAL_TIME_L, Speed);
}

int SCServo::st_WriteSpe(u8 ID, s16 Speed)
{
	if(Speed<0){
		Speed = -Speed;
		Speed |= (1<<10);
	}
	return st_writeWord(ID, P_GOAL_TIME_L, Speed);
}
